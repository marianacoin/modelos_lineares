...
```{r}
data = read_csv("collegiate_athlete_injury_dataset.csv")
dados <- na.omit(data)
dados
```
...
```
O p-valor obtido no teste de Breusch-Pagan foi de 0.4061, indicando que não há evidências suficientes para rejeitar a hipótese nula de homocedasticidade. Portanto, os resíduos podem ser considerados homocedásticos.
O gráfico de resíduos versus valores ajustados foi gerado apresentando os resíduos distribuídos de maneira relativamente aleatória em torno de uma linha de tendência próxima de zero. No entanto, foi possível observar uma leve curvatura no centro, mas não é o suficiente para indicar uma forte violação da homocedasticidade.
